...
/** Builds the drawn rig for `archetype`: the skeleton, body meshes and the style's kit. The framing radius is measured afterwards by `createProceduralFighter`. */
function buildRig(archetype: Archetype): BuiltRig {
  const spec = STYLE_SPECS[archetype]
  const body = spec.body
  const clothColor = spec.clothColor
...
export function createProceduralFighter(options: ProceduralFighterOptions): ProceduralFighter {
  const { root, joints, anchors, owned } = buildRig(options.archetype)
  // The camera's framing input (`ArenaView` -> `ArenaCamera`), measured off the
  // rig that was actually built.
